...
class FirmwareInfoRegressionTests(unittest.TestCase):
    ...
    def test_version_file_is_single_v50_v13_source(self) -> None:
        self.assertEqual(self.version, "V5.0 V13")
        self.assertNotIn("3.0.0-beta.5", self.version)

    def test_release_facing_adaptive_version_is_v50_v13(self) -> None:
        for text in (self.readme, self.nag_operations, self.ui_source, self.ui_base):
            self.assertNotIn("V4.9 V13", text)
            self.assertNotIn("V4.2 V13", text)
        self.assertIn("V5.0-V13", self.readme)
        self.assertIn("V5.0-V13", self.nag_operations)
        self.assertIn("V5.0 V13", self.ui_source)
        self.assertIn("V5.0 V13", self.ui_base)
