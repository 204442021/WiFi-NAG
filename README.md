...
### Mode ADAPTIVE (V4.4-V13 closed loop)

- Requires fresh DAS HOS feedback from read-only `0x39B` and three valid OEM `0x370` frames before sending.
- Treats HOS `0..2` as normal: with the default-on maintenance switch enabled, every valid OEM `0x370` gets a random `1.50..1.80 Nm` echo for about `10 s`, followed by `1..2 s` with no injected frame. Turning maintenance off makes HOS `0..2` monitor-only while correction remains armed.
...
- CAN Write toggle
- Nag mode, A_V2 range, and V4.4-V13 ADAPTIVE closed-loop policy controls
