...
<header class="shell-header" id="wifi-nag-header">
  <div class="brand">
    <div class="brand-copy">
      ...
      <div class="brand-sub"><span class="sdot dot-off" id="dot"></span><span id="hdr-desc">等待设备连接</span><span class="version-badge">V2.1</span><span class="hw-badge" id="hw-badge">WIFI-NAG</span></div>
    </div>
  </div>
...
<main id="wifi-nag-main">
  ...
  <div class="subsec" id="wifi-hotspot-section" data-subkey="config-wifi-hotspot">
    <div class="subsec-head">
      <div class="subsec-title">WiFi Hotspot <span class="title-help" aria-label="Help" onclick="return toggleHelp(this,event)" data-help-target="ap-info" title="Configure the device hotspot password and visibility. The name is fixed.">i</span></div>
      <div class="subsec-meta"><span id="ap-stored" style="margin-right:8px"></span><span id="ap-clients">0 clients</span></div>
    </div>
    <div class="subsec-body">
      <div id="ap-info" class="info-box" style="display:none">热点名称固定为 Albert-FSD；密码和隐藏状态会保存在设备中。</div>
      <div class="setting-desc" style="margin-bottom:8px">热点名称固定，只能修改密码</div>
      <div style="display:flex;gap:6px;margin-bottom:6px">
        <input class="sniff-input" id="ap-ssid" value="Albert-FSD" disabled readonly style="flex:1">
  ...
  <div class="subsec diagnostic-overview">
    ...
    <div class="stat-grid" id="status-panel">
      ...
      <div class="stat"><div class="stat-lbl">系统版本</div><div class="stat-val v-dim" id="diag-version">V2.1</div></div>
    </div>
...
function syncDashboardSummary(){
  ...
  mirrorDashboardText('fw-version','diag-version','V2.1');
...
async function saveAP(){
  const pass=$('ap-pass').value,hidden=$('ap-hidden').checked?'1':'0';
  if(pass&&pass.length<8){$('ap-status').textContent=trText('Password min 8 chars');$('ap-status').style.color='var(--err)';return;}
  try{const r=await fetch('/ap_config',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'pass='+encodeURIComponent(pass)+'&hidden='+hidden});
    const d=await r.json();
