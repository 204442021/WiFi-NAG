...
/* OTA upload */
.ota-info{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-bottom:12px}
.ota-info-item{min-width:0;padding:9px 10px;border:1px solid var(--bd);border-radius:8px;background:var(--bg2)}
.ota-info-label{font-size:10px;color:var(--tx3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px}
.ota-info-value{font-size:12px;font-weight:700;color:var(--tx);font-family:'SF Mono','Courier New',monospace;word-break:break-word}
...
<div class="card" id="firmware-update-card">
  ...
  <div style="margin-top:4px">
    <div class="ota-info">
      <div class="ota-info-item"><div class="ota-info-label">Firmware Version</div><div class="ota-info-value" id="ota-version">--</div></div>
      <div class="ota-info-item"><div class="ota-info-label">Partition Size</div><div class="ota-info-value" id="ota-partition-size">--</div></div>
      <div class="ota-info-item"><div class="ota-info-label">Current Partition</div><div class="ota-info-value" id="ota-current-partition">--</div></div>
      <div class="ota-info-item"><div class="ota-info-label">Target Partition</div><div class="ota-info-value" id="ota-target-partition">--</div></div>
    </div>
...
// OTA upload
function otaPartitionText(label,slot,address){
  if(!label)return trText('unavailable');
  const otaName=Number(slot)>=0?'ota_'+Number(slot):'?';
  return label+' ('+otaName+') @ '+fmtAddr(address);
}

function otaPartitionSizeText(currentSize,targetSize){
  const sizeText=n=>Number(n)>0?(Number(n)/1048576).toFixed(2)+' MiB':'--';
  return sizeText(currentSize)+' / '+sizeText(targetSize);
}

async function loadOtaStatus(){
  try{
    const d=await fetchPollJson('/ota_status',2500);
    setText('fw-ver',d.firmware||trText('Manual OTA'));
    setText('ota-version',d.firmware||'--');
    setText('ota-current-partition',otaPartitionText(d.current_label,d.current_slot,d.current_address));
    setText('ota-target-partition',otaPartitionText(d.target_label,d.target_slot,d.target_address));
    setText('ota-partition-size',otaPartitionSizeText(d.current_size,d.target_size));
  }catch(e){
    setText('fw-ver',trText('Manual OTA'));
    ['ota-version','ota-current-partition','ota-target-partition','ota-partition-size'].forEach(id=>setText(id,'--'));
  }
}
...
document.addEventListener('visibilitychange',()=>{
  ...
  poll();loadWifiStatus();loadApStatus();loadGatewayStatus();loadOtaStatus();
  if(!networkPerformanceMode&&!isCarUiActive()){loadWifiNetworks();loadGatewayBlocked();loadGatewayDns(true);}
  pollLog();
});
