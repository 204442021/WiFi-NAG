...
- HOS 0..1 使用 `MAINTENANCE` 在 `1.50 .. 1.80 Nm` 范围内维持注入。每次 Echo 成功后力度严格变化 `0.01 Nm`，到配置边界后反向。预防停发默认 `0/0`，表示关闭停发并持续扫动；也可在页面改为非零的注入/停发周期。关闭维持开关后 H0～H2 只监控，H3～H5 纠正仍然工作。
- HOS 2 使用独立连续检测计时器，默认 3 秒，不会覆盖或重启当前 `MAINTENANCE` / `REST` 预防周期。回到 H0..1 会清零；超时后进入 `PRE_CORRECTIVE_PAUSE`，停止全部自适应发送 1 秒，再进入纠正。HOS 3..5 不等待 H2 计时，立即进入同一停发阶段。
- `CORRECTIVE` 首方向按方向盘角度的反方向确定。正负帧数都大于 0 时执行双向三角扫并用固定 10 个成功 Echo 平滑换向；某侧帧数为 0 时关闭该侧并固定另一方向；两侧都为 0 时纠正层只监控、不发送。每个有效方向默认 100 个成功发送帧，从所设最小值升至随机峰值再回到最小值；峰值默认 `1.80 .. 2.40 Nm`，可设范围上限为 `2.50 Nm`。进入默认 1～2 秒的 `VERIFY` 停发时保留当前方向、帧位和峰值，恢复后从原位置继续。
- HOS 2 不算纠正成功；必须连续两帧回到 H0..1 才退出。随后立即恢复维持注入并进入 `STABILITY_VERIFY`，默认连续确认 H0..1 维持 5 秒；期间出现 H2/H3 会重新进入恢复循环。
- HOS 6..15 均 fail-closed 并进入 `FAULT_HOLD` 保护停发，其中 9..14 为未定义状态；DAS stale 同样阻止发送。HOS 0..2 连续稳定 2000 ms 后自动恢复，显式 reset 立即 off/on。
- 方向判断只使用方向盘角度：大于 `+1.0°` 选择负向注入，小于 `-1.0°` 选择正向注入，中间区保留上次方向；纠正首方向确定后，由所配置的固定单向或双向扫动状态机控制。实测原车扭矩只保留为诊断数据。
- 只有纠正区可设置到 `±2.50 Nm`，默认峰值范围为 `1.80 .. 2.40 Nm`；维持区和旧模式仍限制在 `±1.80 Nm`。方向盘角度大于 `+1.0°` 时注入负扭矩，小于 `-1.0°` 时注入正扭矩，中间区保留上次方向；冷启动位于中间区时不发送。角度绝对值大于 `50.0°` 停发，恰好 `±50.0°` 允许发送。
- 仅保留即时 Echo：每个有效、非本机回显的 OEM `0x370` 最多立即发送一个 Counter+1 Echo；不再包含 Late Echo、纠正最小发送间隔、延迟发送或重试。
