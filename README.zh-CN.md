...
- HOS 0..1 使用 `MAINTENANCE` 在 `1.70 .. 1.80 Nm` 范围内维持注入。每次 Echo 成功后力度严格变化 `0.01 Nm`，到配置边界后反向。预防停发默认 `0/0`，表示关闭停发并持续扫动；也可在页面改为非零的注入/停发周期。关闭维持开关后 H0～H2 只监控，H3～H5 纠正仍然工作。
- HOS 2 进入 `H2_PENDING` 连续检测，默认 3 秒。回到 H0..1 会清零；超时后进入 `PRE_CORRECTIVE_PAUSE`，停止全部自适应发送 500 ms，再进入纠正。HOS 3..5 不等待 H2 计时，立即进入同一停发阶段。
- `CORRECTIVE` 每个发送窗口先按当前可信原车扭矩确定首方向，随后与实测扭矩方向解耦，按负、正两侧独立帧数交替做非零三角扫。每侧默认 50 个成功发送帧，从接近 0 平滑升至随机峰值再回落；峰值默认仍为 `1.80 .. 2.00 Nm`，可设范围扩到 `2.50 Nm`。仍未解除则进入 `VERIFY` 停发 1～2 秒后继续。
...
- 方向判断不再保留死区字段：第一个非零原车扭矩立即确定反向注入方向；维持区反向变化仍需稳定 100 ms。纠正发送窗口锁定首方向后，由独立正负扫动状态机控制。
- 只有纠正区可设置到 `±2.50 Nm`，默认仍为 `1.80 .. 2.00 Nm`；维持区和旧模式仍限制在 `±1.80 Nm`。counter、checksum、own-echo 跳过及 stale/fault no-send 约束保持不变。
- 可选 Late Echo 默认关闭。开启后先学习 8 个有效 OEM 周期，再在预测的下一原车帧前约 1 ms 安排一个 Counter+1 Echo；新 OEM 帧会替换旧待发送帧，写入关闭、OTA/重启准备、DAS stale 或 H6+ 会清空。该时序模式为实验功能，本版本不声称已完成实车验证。
