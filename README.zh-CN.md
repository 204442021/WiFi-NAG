...
- HOS 0..2 属于正常范围，HOS 2 是当前系统常态；默认开启“维持区”开关，`MAINTENANCE` 对每个有效原车 `0x370` 注入随机 `1.50 .. 1.80 Nm`，持续 1～2 秒。关闭后 H0～H2 只监控，H3～H5 纠正仍然工作。
- 预防注入结束后进入 `REST`，连续 3～5 秒不额外发送 `0x370`，随后自动开始下一轮 1～2 秒注入；不会注入 `0 Nm`。
- HOS 3..5 进入 `CORRECTIVE`：先清除上一目标，立即注入随机 `1.80 .. 2.00 Nm`，持续 1 秒；仍未回到 H0..2 时进入 `VERIFY` 完全停发 500 ms，然后继续下一轮 1 秒纠正，循环直到降级。降级后先进入预防层 3～5 秒停发期。
