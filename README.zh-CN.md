...
- HOS 0..1 使用 `MAINTENANCE` 维持注入 `1.70 .. 1.80 Nm`。预防停发默认 `0/0`，表示关闭停发并持续注入；也可在页面改为非零的注入/停发周期。关闭维持开关后 H0～H2 只监控，H3～H5 纠正仍然工作。
...
- `CORRECTIVE` 每个发送窗口只选择一次 `1.80 .. 2.00 Nm` 并保持不变，默认发送 3 秒且成功发送间隔为 1 ms；仍未解除则进入 `VERIFY` 停发 1～2 秒后继续。纠正停发最小/最大都填 0 可关闭停发。
...
- 方向判断不再保留死区字段：第一个非零原车扭矩立即确定反向注入方向；反向变化仍需稳定 100 ms，确认期间停止发送旧方向。
