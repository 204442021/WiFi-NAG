...
- HOS 2 进入 `H2_PENDING` 连续检测，默认 3 秒。回到 H0..1 会清零；超时后进入 `PRE_CORRECTIVE_PAUSE`，停止全部自适应发送 1 秒，再进入纠正。HOS 3..5 不等待 H2 计时，立即进入同一停发阶段。
- `CORRECTIVE` 每个发送窗口先按方向盘角度的反方向确定首方向，随后锁定为独立双向三角扫。每侧默认 100 个成功发送帧，从接近 0 平滑升至随机峰值再回落；峰值默认 `1.80 .. 2.40 Nm`，可设范围上限为 `2.50 Nm`。仍未解除则进入 `VERIFY` 停发 1～2 秒后继续。
...
- 方向判断只使用方向盘角度：大于 `+1.0°` 选择负向注入，小于 `-1.0°` 选择正向注入，中间区保留上次方向；纠正发送窗口锁定首方向后，由独立正负扫动状态机控制。实测原车扭矩只保留为诊断数据。
