...
- HOS 0..2 属于正常范围，HOS 2 是当前系统常态；默认开启“维持区”开关，`MAINTENANCE` 对有效原车 `0x370` 注入 `1.70 .. 1.80 Nm`，默认持续 2～3 秒。关闭后 H0～H2 只监控，H3～H5 纠正仍然工作。
- 预防注入结束后进入 `REST`，默认连续 4～5 秒不额外发送 `0x370`，随后自动开始下一轮 2～3 秒注入；不会注入 `0 Nm`。
- HOS 3..5 进入 `CORRECTIVE`：先清除上一目标，每个纠正发送窗口只选择一次 `1.80 .. 2.00 Nm` 并保持不变，成功发送默认限制为每 1 ms 一帧；默认固定发送 3 秒，仍未回到 H0..2 时进入 `VERIFY` 完全停发 1～2 秒，然后继续下一轮，循环直到降级。降级后先进入预防层 4～5 秒停发期。纠正发送、停发和帧间隔均可在页面独立调节。
